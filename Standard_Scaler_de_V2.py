...
REESCALANDO LOS DATOS: Mismo que la version 2 pero reescalando los datos
'''
...
conf=SparkConf().setAppName("Machine_Learning_ORCs_Scaler")
...
numeric=['edad', 'antiguedad_cliente', 'capacidad_pago', 'monto_del_credito', 'duracion_credito', 'creditos_pagados', 'creditos_no_pagados']

df_scaled=df_pd[numeric]
from sklearn.preprocessing import StandardScaler
scaler=StandardScaler()
scaler.fit(df_scaled)
df_scaled=scaler.transform(df_scaled) 
df_scaled=pd.DataFrame(df_scaled, columns=numeric)
df_pd=pd.concat([df_pd.drop(df_pd[numeric], axis = 1).reset_index(drop=True), df_scaled], axis=1)
# Dimesion  (1280402, 16)
